...
The critical path of every page on this site - HTML, inline critical CSS, and any inlined SVG - **must fit under 14 KB, Brotli-q11 compressed**. External CSS, images, and any JavaScript are deferred and cannot block first paint. No web fonts, ever.
...
| Web fonts shipped, ever               | **0 B**                   | System stack only; no @font-face anywhere  |
| Total page weight                     | <= 150 KB                 | All assets, all requests, cumulative       |
...
The theme ships no JS by default. Any feature that would need JS must justify its bytes against a user benefit, not against "it's nice to have."
...
- Hashed, content-addressable filenames for every asset (`style.<sha256>.css`, etc.). Hugo's `resources.Fingerprint` handles this.
- `Cache-Control: public, max-age=31536000, immutable` on all hashed assets (1 year, never revalidated).
- `Cache-Control: public, s-maxage=300, stale-while-revalidate=60` on HTML, paired with a zone-level **Cache Rule** that marks HTML paths on the production hostname as eligible for edge caching with "use cache-control header from origin". Without the Cache Rule, Cloudflare's default cache level skips HTML regardless of the directive. With both in place, CF edge caches HTML for 5 minutes and emits `Last-Modified`, so browser revalidation via `If-Modified-Since` returns `304 Not Modified` - typically 0 bytes over the wire after the first visit. `stale-while-revalidate` lets the edge refresh in the background so deploys propagate within one client request after the TTL lapses.
- `_headers` file in the repo root configured for Cloudflare Pages.

## DNS, TLS, and transport

- **DNS**: authoritative on Cloudflare. The production record is a CNAME to the Pages hostname, proxied (orange cloud) in production.
- **TTL**: 60 s during any cutover, 3600 s in steady state.
...
4. **Defer aggressively.** JavaScript, non-critical CSS, images below the fold, analytics - all deferred, all optional, all removable without breaking the page. No web fonts in any tier - system stack only.
5. **Progressive enhancement, not progressive degradation.** The page must work with JavaScript disabled and images failing. Everything above that is decoration.
6. **No third-party requests, ever.** Zero. No Google Analytics, no Disqus, no fonts from Google Fonts, no CDN-hosted anything. The browser talks to one origin and that is it.
...
If a PR violates any rule above:
